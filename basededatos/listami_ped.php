...
$sqlin="SELECT ID_INSUMO,nombre,cantidad,descripcion,precio,iva FROM Insumo,Proveedor WHERE Insumo.estado='1' AND ID_PROVEEDOR=". $prove ." ORDER BY Insumo.nombre ASC;";
$resultin=mysqli_query($conn,$sqlin);
		...
		<tr align="center">
			...
			<td><?php echo $nomm; ?></td>
			<td><?php echo $desm; ?></td>
			<td><?php echo $prem; ?><input type="hidden" name="pre[]"></input></td>
			<td><?php echo $ivam; ?></td>
			<td><?php echo $canm." ".$nommc; ?></td>
			<td>Materia prima<input type="hidden" name="tip[]"></input></td>
...
<?php }
		...
		<tr align="center">
			...
			<td><?php echo $nomi; ?></td>
			<td><?php echo $desi; ?></td>
			<td><?php echo "$".$prei; ?><input type="hidden" name="pre[]"></input></td>
			<td><?php echo $ivai."%"; ?></td>
			<td><?php echo $cani." Unidades"; ?></td>
			<td>Insumo<input type="hidden" name="tip[]"></input></td>
			<td><input type="number" name="cant[]" id="cant" class="form-control" maxlength="3" oninput="maxlengthNumber(this)" onpaste="return false" onkeypress="return validanumericos(event)" disabled="" required=""></td>				
		</tr> <label></label>
<?php }} ?>
